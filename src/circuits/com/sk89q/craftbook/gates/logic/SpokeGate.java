...
public class SpokeGate extends AbstractIC implements MultiInputIC,
	...
	public void trigger(final ChipState chip) {
		try {
			...
			Chunk chunk = world.getBlockAt(0, 0, 0).getChunk();
			...
			int startX = -20, startY = 80, startZ = 50;

			int cX = 0, cZ = 0;

			for (int x = 0; x < width; x += scale) {
				cZ = 0;
				for (int z = 0; z < height; z += scale) {
					System.out.print(x + " " + z);
					int[] fc = new int[scale * scale];
					Chunk cnk=null;
					for (int iX = 0; iX < scale; iX++) {
						for (int iZ = 0; iZ < scale; iZ++) {
							for (int i = 127; i >= 0; i--) {
								Block blk;
								int type = (blk=world.getBlockAt(x + iX, i, z + iZ))
										.getTypeId();
 								if (type == 0)
									continue;
								fc[iX + iZ * scale] = type;
					...
					world.getBlockAt(startX + cX, startY, startZ + cZ)
							.setTypeId(last);
					world.getBlockAt(startX + cX, startY - 1, startZ + cZ)
							.setTypeId(49);
