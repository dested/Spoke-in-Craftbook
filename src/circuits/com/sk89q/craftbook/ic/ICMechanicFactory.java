...
public class ICMechanicFactory extends AbstractMechanicFactory<ICMechanic> {
	 ...
	 */
	public static final Pattern codePattern = Pattern.compile(
			"^\\[(MC[^\\]]+)\\]$", Pattern.CASE_INSENSITIVE);
	...
	public ICMechanic detect(BlockWorldVector pt)
		...
		// after this point, we don't return null if we can't make an IC: we
		// throw shit,
		// because it SHOULD be an IC and can't possibly be any other kind of
		// mechanic.
	...
	//	if (ic instanceof MemoryManaged) {
	//		((MemoryManaged) ic).setMemoryBank(sign.getLine(0).replace(ic.getSignTitle(), ""));
	//	}

		// okay, everything checked out. we can finally make it.
		if (ic instanceof SelfTriggeredIC) {
			return new SelfTriggeredICMechanic(plugin, id,
					(SelfTriggeredIC) ic, registration.getFamily(), pt);
		} else if (ic instanceof MultiInputIC) {
			return new MultiInputICMechanic(plugin, id, (MultiInputIC) ic,
					registration.getFamily(),
					((MultiInputIC) ic).getTriggerPositions(pt, null, sign));
		} else {
			return new ICMechanic(plugin, id, ic, registration.getFamily(), pt);
		}
	}
	...
	public ICMechanic detect(BlockWorldVector pt, LocalPlayer player, Sign sign)
		...
		if (block.getTypeId() != BlockID.WALL_SIGN) {
			throw new InvalidMechanismException(
					"Only wall signs are used for ICs.");
		}
		...
		if (factory instanceof RestrictedIC) {
			if (!player.hasPermission("craftbook.ic.restricted."
					+ id.toLowerCase())) {
		...
		ICMechanic mechanic;
		if (ic instanceof SelfTriggeredIC) {
			mechanic = new SelfTriggeredICMechanic(plugin, id,
					(SelfTriggeredIC) ic, registration.getFamily(), pt);
		} else if (ic instanceof MultiInputIC) {
			mechanic = new MultiInputICMechanic(plugin, id, (MultiInputIC) ic,
					registration.getFamily(),
					((MultiInputIC) ic).getTriggerPositions(pt, player, sign));
		} else {
			mechanic = new ICMechanic(plugin, id, ic, registration.getFamily(),
					pt);
		}

		if (ic instanceof MemoryManaged) { 
			sign.setLine(0, sign.getLine(0) + ic.getSignTitle());
		} else {
			sign.setLine(0, ic.getSignTitle());
		}

		player.print("You've created " + registration.getId() + ": "
				+ ic.getTitle() + ".");
